...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn)
{
...

    {
        ...
        txNew.vout[0].nValue = nFees + GetFartcoinBlockSubsidy(nHeight, consensus, pindexPrev->GetBlockHash());
...
static bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("FartcoinMiner: generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock, true, NULL))
        return error("FartcoinMiner: ProcessNewBlock, block not accepted");
...
{
    LogPrintf("FartcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("fartcoin-miner");
    const CChainParams& chainparams = Params();
    ...
    try {
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("Error in FartcoinMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("Running FartcoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("FartcoinMiner terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("FartcoinMiner runtime error: %s\n", e.what());
        return;
    }
}

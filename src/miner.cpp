...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (auxpow != NULL) {
        ...
        // print to log
        LogPrintf("FartcoinMiner: AUX proof-of-work found; our hash: %s ; parent hash: %s ; target: %s\n",
               hash.GetHex().c_str(),
    ...
    else
    {
        ...
        // print to log
        LogPrintf("FartcoinMiner: proof-of-work found; hash: %s ; target: %s\n", hash.GetHex().c_str(), hashTarget.GetHex().c_str());
    }
    ...
    // Found a solution
    {
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FartcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static FartcoinMiner(CWallet *pwallet)
{
    ...
    try { while (true) {
        ...
        LogPrintf("Running FartcoinMiner with %" PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet, int nThreads)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FartcoinMiner, pwallet));
}

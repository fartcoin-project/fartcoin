    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional FART outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes FART and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::FART_TO_FART) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        // First try to construct a FART-to-FART transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::FART_ONLY;
        ...
        // If FART-to-FART fails, create a peg-out transaction

...
//rnicoll: 8/2021 reduce to 1,000,000 Farticle
...
/* 1.14.5: set the wallet's discard threshold to 1 FART because that's what 97%
 ...
 */
/* 1.14.6: set the wallet's discard threshold to 0.01 FART. Very network
...
/*
 * Fartcoin: Scaled to 1/10th of the recommended transaction fee to make RBF
...
/*
 * Fartcoin: Creating change outputs at exactly the dustlimit is counter-
...
public:
    ...
    /**
     * Fartcoin: Get a fee targetting a specific transaction speed.
     */
    CAmount GetFartcoinPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nSpeed);
    /**
     * Fartcoin: Get a fee targetting a specific transaction speed.
     */
    static CAmount GetFartcoinPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nSpeed, CAmount targetFee);
    /**
    ...
    /**
     * Return the minimum required fee taking into account the
     * floating relay fee and user set minimum transaction fee, but not the Fartcoin dust fee.
     */

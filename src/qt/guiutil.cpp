...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Fartcoin address (e.g. FHN3ZNETUqim2aenVwyQH6WBjgBi6NAM5M)"));
#endif
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                ...
                if(!BitcoinUnits::parse(BitcoinUnits::FART, i->second, &rv.amount, locale))
                {
                    return false;
                }
            }
...
{
    // Convert fartcoin:// to fartcoin:wow
    //
...
{
    QString ret = QString("fartcoin:%1").arg(info.address);
    int paramCount = 0;
    ...
    if (info.amount)
    {
        ...
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::FART, info.amount, false, true, localeC));
        paramCount++;
    }

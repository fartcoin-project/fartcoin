    ...
    : m_nFeePaid(nFeePaid), m_nBytes(nBytes_), m_weight(mweb_weight)
{
    ...
    if (mweb_fee > 0 && nFeePaid < mweb_fee) {
        ...
        if (nSize > 0)
            nFarticlesPerK = ltc_fee * 1000 / nSize;
        else
            nFarticlesPerK = 0;
    }
}
...
CAmount CFeeRate::GetFee(size_t nBytes_) const
{
    ...
    CAmount nFee = nFarticlesPerK * nSize / 1000;
...
bool CFeeRate::MeetsFeePerK(const CAmount& min_fee_per_k) const
{
    ...
    // Anything beyond that can be used to calculate nFarticlesPerK.
    CAmount mweb_fee = CAmount(m_weight) * BASE_MWEB_FEE;
    if (m_weight > 0 && m_nFeePaid < mweb_fee) {
        return false;
    }

    // MWEB-to-MWEB transactions don't have a size to calculate nFarticlesPerK.
    // Since we got this far, we know the transaction meets the minimum MWEB fee, so return true.
    if (m_nBytes == 0 && m_weight > 0) {
        return true;
    }

    return nFarticlesPerK >= min_fee_per_k;
}

std::string CFeeRate::ToString(const FeeEstimateMode& fee_estimate_mode) const
{
    switch (fee_estimate_mode) {
    case FeeEstimateMode::SAT_VB: return strprintf("%d.%03d %s/vB", nFarticlesPerK / 1000, nFarticlesPerK % 1000, CURRENCY_ATOM);
    default:                      return strprintf("%d.%08d %s/kvB", nFarticlesPerK / COIN, nFarticlesPerK % COIN, CURRENCY_UNIT);
    }
}

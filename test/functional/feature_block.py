...
class FullBlockTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Create a transaction that spends one farticleto the p2sh_script, the rest to OP_TRUE
        ...
        # Until block is full, add tx's with 1 farticleto p2sh_script, the rest to OP_TRUE
        tx_new = None
        tx_last = tx
        ...
        #       this fails because the coinbase is trying to claim 1 farticletoo much in fees
        #
        # b69 - coinbase with extra 10 satoshis, and a tx that gives a 10 farticlefee
        #       this succeeds
        #
    ...
    def next_block(self, number, spend=None, additional_coinbase_value=0, script=CScript([OP_TRUE]), *, version=0x20000000):
        ...
        if spend is None:
            ...
            coinbase.vout[0].nValue += spend.vout[0].nValue - 1  # all but one farticleto fees
            coinbase.rehash()
            block = create_block(base_block_hash, coinbase, block_time, version=version)

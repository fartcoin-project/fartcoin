...
3. All content of the data directory, except for the `fartcoin.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`./`               | `fartcoin.conf`       | User-defined configuration settings for `fartcoind` or `fartcoin-qt`; can be specified by `-conf` option
`./`               | `fartcoind.pid`        | Stores the process ID (PID) of `fartcoind` or `fartcoin-qt` while running; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `fartcoind` or `fartcoin-qt`
...
`fartcoin-qt` uses the [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by Fartcoin Core:

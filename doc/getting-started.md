...
This tutorial will help you to go through the basics to use Fartcoin Core after you completed the [installation instructions](/INSTALL.md). You now have `fartcoind` or `fartcoin-qt` executables available to run a node, and `fartcoin-cli`/`fartcoin-tx` tools to help you transact FART.
...
> ```console
> shibetoshi:~$ /path/to/fartcoin-cli [arguments ...]
> ```

### Table of contents

1. [Starting a fartcoin node](#starting-a-fartcoin-node)
...
## Starting a Fartcoin node

To start your node, you can run an headless server using `fartcoind`:
```console
shibetoshi:~$ fartcoind -daemon
```

Or you can use the Graphical User Interface (GUI), `fartcoin-qt`:
```console
shibetoshi:~$ fartcoin-qt
```
...
Your node is now running and starts with a *synchronization process* that downloads the entire blockchain from other nodes. This operation will take many hours to complete, but you are now part of the Fartcoin network!

> **Note:** The rest of this guide assumes the use of an headless node. The RPC server is not exposed with `fartcoin-qt` until you activate the `-server` option as a startup argument, but inside the GUI application, you can use all the commands explored below (without `fartcoin-cli`) by going to `Help -> Debug window` and inside the popup window selecting the tab `Console`.

## Introduction to the JSON-RPC API

Fartcoin Core exposes a JSON-RPC interface that allows you to request information about the network, blockchain and individual transactions, send transactions to the networks and manage your wallet.

The Fartcoin Core installation provides the `fartcoin-cli` tool to interact with the JSON-RPC from the command line, and the interface is exposed over HTTP on port `13377`, so that other tools and libraries can interact with it.

To have an overview of the available commands, use the `help` command:

```console
#List all commands
shibetoshi:~$ fartcoin-cli help

#Get help for a specific command
shibetoshi:~$ fartcoin-cli help COMMAND
```
...
To receive FART, you need an address that is securely derived from a private key through a series of automatic, cryptographic operations. The *address* can be shared with anyone to receive FART, but the *private key* is  sensitive information that allows anyone that knows it to spend the FART on the associated address.

By default, the Fartcoin Core software will automatically create an address for you and securely store the private key in the wallet file.

You can list wallet addresses using `getaddressesbyaccount`:

```console
shibetoshi:~$ fartcoin-cli getaddressesbyaccount ""
[
...
```console
shibetoshi:~$ fartcoin-cli getnewaddress
...
```console
shibetoshi:~$ fartcoin-cli backupwallet /path/of/wallet/backup
```

**Tip:** Fartcoin addresses start with the letter `D`.

You now have two wallet addresses to share with other people to receive FART! Consider avoiding [address reuse](https://en.bitcoin.it/wiki/Address_reuse) for anonymity and security reasons.
...
```console
#Syntax
shibetoshi:~$ fartcoin-cli getbalance "*" minconf
```
...
```console
#Syntax
shibetoshi:~$ fartcoin-cli sendtoaddress address amount

#Example
shibetoshi:~$ fartcoin-cli sendtoaddress nWSYUqtimF7B6qW4GBdczaG6jvqKutS1Nh 420
```
...
```console
#Syntax
shibetoshi:~$ fartcoin-cli listunspent minconf maxconf '["address", ...]'

#Example
shibetoshi:~$ fartcoin-cli listunspent 1 9999999 '["nnJDY1xCRgWQc7vBXHUPMPsEynuZW23Y3P"]'
[
  {
...
shibetoshi:~$ fartcoin-cli createrawtransaction "$utxos_to_use" '{"address":amount, ...}'
...
shibetoshi:~$ fartcoin-cli createrawtransaction "$utxos_to_use" '{"nWSYUqtimF7B6qW4GBdczaG6jvqKutS1Nh":69, "nnJDY1xCRgWQc7vBXHUPMPsEynuZW23Y3P": 30.999}'
...
> **Tip:** The transaction returned is encoded in hexadecimal encoding. You can use `fartcoin-cli decoderawtransaction` or `fartcoin-tx -json` to convert the content to JSON format.

##### signrawtransaction

Before sending a transaction, it must be signed by the private key that the address was derived from. Fartcoin Core will automatically use the correct private key when spending UTXO known to the wallet.

```console
#Syntax
shibetoshi:~$ fartcoin-cli signrawtransaction encoded_transaction

#Example
shibetoshi:~$ fartcoin-cli signrawtransaction "01000000013324faf8f03695261611669f5bdd93b68a86cf02db12dc46642ed50666ed69b80000000000ffffffff0200a5459b010000001976a91418a89ee36293f15c4db4c01173babd579243161188ac60b8c4b8000000001976a914c6977da37560e1432c2e14e16952981a4c272cac88ac00000000"
{
...
```console
#Syntax
shibetoshi:~$ fartcoin-cli sendrawtransaction signed_transaction

#Example
shibetoshi:~$ fartcoin-cli sendrawtransaction 
...
> **Note:** To be able to query transactions not related to your own wallet, like in this example, you will need to enable the `-txindex` option. This options requires the Fartcoin Core software to re-index the entire blockchain, and can take up to several hours.

First, request the information about block 69:

```console
#Find block hash from his height
shibetoshi:~$ fartcoin-cli getblockhash 69
3d2def20cd0d3aca148741ef469bda11647a3040d7669c82745d03c728706a8b

#Get block data
shibetoshi:~$ fartcoin-cli getblock 3d2def20cd0d3aca148741ef469bda11647a3040d7669c82745d03c728706a8b
{
...
```console
#Syntax
shibetoshi:~$ fartcoin-cli getrawtransaction txid verbose

#Example
shibetoshi:~$ fartcoin-cli getrawtransaction 695ce4208fa7a87ef9e99805b0910dc129058ecdceb5cef7e25f71dcdc7936db 1
{
...
Using `fartcoind -help` will display all available configuration parameters that can be added as arguments:

**Command example :**
```console
shibetoshi:~$ fartcoind -daemon -paytxfee=0.01 -sendfreetransactions=1 -maxconnections=150
```

Configuration can be persisted by creating a `fartcoin.conf` file. Create it in the directory defined with the `datadir` setting, `$HOME/.fartcoin` by default, or specify the file location with `-conf`.

**fartcoin.conf example :**
```
daemon=1
server=1
listen=1
...
maxconnections=150
```
You can see a more concrete example [here](/contrib/debian/examples/fartcoin.conf).

### Mainnet, testnet and regtest

When trying out new things, for example to test your application that interacts with the Fartcoin chain, it is recommended to not use the main Fartcoin network. Multiple networks are built-in for this purpose.
...
When not specifying any network, *Mainnet* is the network used by default. To enable *testnet*, use the `fartcoind -testnet`.

To enable *regtest*, use the `-regtest` option.

> **Tip:** Remember to specify the network when you want to use `fartcoin-cli`.

### Data directory

The data directory is the location where Fartcoin Core files are stored, including the wallet, log files and blocks. You can modify the location with the `-datadir` setting.
...
Running Fartcoin Core can require a lot of memory, so in some situations it may be necessary to optimize its usage. You can find more information about reducing the memory footprint in the [related guide](reduce-memory.md).

### Troubleshooting

By default, Fartcoin Core keeps detailed logs in the `debug.log` file, located in the `datadir`. Alternatively, the `-printtoconsole` parameter displays the log interactively to the terminal instead.

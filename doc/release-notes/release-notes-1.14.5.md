Fartcoin Core version 1.14.5 is now available from:

  <https://github.com/fartcoin/fartcoin/releases/tag/v1.14.5/>

This is a new minor version release, including important security updates and
changes to network policies. All Fartcoin Core users, miners, services, relay
operators and wallet users are strongly recommended to upgrade.

Please report bugs using the issue tracker at github:

  <https://github.com/fartcoin/fartcoin/issues>

To receive security and update notifications, please watch reddit or Twitter:

  * https://www.reddit.com/r/fartcoindev/
  * @Fartcoin on Twitter for high priority announcements
  * @fartcoin\_devs on Twitter for updates on development work

Compatibility
==============

Fartcoin Core is extensively tested on Ubuntu Server LTS, Intel-based macOS
and Windows 10.
...
most Fartcoin Core users.

### Remote Code Execution in Fartcoin QT (CVE-2021-3401)
...
**Fartcoin QT users are urged to please update their installations to this
...
leakage of sensitive information when Fartcoin Core crashes on Unix platforms.
The vulnerability is patched for systems that run a Linux kernel equal to or
higher than 3.4.

**Fartcoin Core wallet users on Linux platforms are urged to please update to
this version.**
...
the Fartcoin network, following the reduction of relay and mining defaults in
...
* the recommended minimum transaction fee is 0.01 FART/kb, and
* the recommended dust limit is 1 FART, and
* the recommended RBF increment is 0.001 FART.

### Wallet/UI Changes

* The user interface for selecting fees when transacting FART has been updated
  to give an idea of how much is being spent, rather than a block target. As
  Fartcoin blocks are not full, typically all transactions are mined in the next
  block, and therefore the target estimation does not makes sense for Fartcoin.
...
* The default minimum transaction fee is now 0.01 FART per kilobyte. Note that
...
* Introduce `-discardthreshold`, a wallet-specific, configurable dust limit that
  ...
  default dust limit is recommended to stay at 1 FART, as versions 1.14.2 until
  1.14.4 have a bug that rejects any transaction with an output under 1 FART.
...
* Split the dust limit into a hard and soft threshold, to reintroduce the
  ...
  * `-harddustlimit` is by default set at 0.001 FART and sets the value under
...
The Berkley DB version used by Fartcoin Core has been updated to 5.3 (from 5.1)
...
performance of cryptographic routines within Fartcoin Core.

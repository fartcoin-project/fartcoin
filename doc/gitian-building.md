...
`gitian-build.sh` is a standalone script, it can be downloaded and run outside of Fartcoin Core repository.
...
gpg --detach-sign ${VERSION}-win/${SIGNER}/fartcoin-win-build.assert
...
Gitian signatures for each release are added to https://github.com/fartcoin/gitian.sigs.

`gitian-build.sh` will create signatures inside `gitian-output/sigs/` folder. Create a pull request to [fartcoin/gitian.sigs](https://github.com/fartcoin/gitian.sigs) to publish your signatures, the `.assert` and `.assert.sig` files.

**When your PR is merged, you will be recorded for all future history as a *Gitian Builder of Fartcoin Core*!**

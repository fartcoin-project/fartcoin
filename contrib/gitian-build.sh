...
# Dependencies
ossPatchUrl="https://depends.fartcoincore.org/osslsigncode-Backports-to-1.7.1.patch"
ossPatchHash="a8c4e9cafba922f89de0df1f2152e7be286aba73f78505169bc351a7938dd911"

ossTarUrl="https://depends.fartcoincore.org/osslsigncode_1.7.1.orig.tar.gz"
ossTarHash="f9a8cdb38b9c309326764ebc937cba1523a3a751a7ab05df3ecc99d18ae466c9"

macosSdkUrl="https://depends.fartcoincore.org/MacOSX10.11.sdk.tar.gz"
...
SIGNER=
VERSION=
url=https://github.com/fartcoin/fartcoin
proc=2
mem=2000
...
Standalone script to perform the gitian build of Fartcoin Core. Perform
...
-u|--url repo       Specify the URL of the repository. Default is https://github.com/fartcoin/fartcoin
...
function move_build_files() {
    find build/out -type f -exec mv '{}' $outputDir/fartcoin-binaries/${VERSION}/ \;
}
...
if [[ $setup == true ]]; then
    git clone https://github.com/fartcoin/gitian.sigs.git
    git clone https://github.com/fartcoin/fartcoin-detached-sigs.git
...
if [[ $build == true ]]; then
    # Make output folder
    mkdir -p $outputDir/fartcoin-binaries/"$VERSION"

    pushd ./gitian-builder || exit 1

    # Clean fartcoin git directory because of old caching
    if [ -d inputs/fartcoin/ ]; then
        echo "Cleaning Fartcoin directory..."
        rm -rf inputs/fartcoin/
    fi
    ...
    for descriptor in "${DESCRIPTORS[@]}"; do
        ...
        echo ""
        ./bin/gbuild -j "$proc" -m "$mem" --commit fartcoin="$COMMIT" --url fartcoin="$url" ../gitian-descriptors/gitian-"$descriptor".yml  || exit 1
